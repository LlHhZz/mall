package com.kob.backend.controller.order;

import com.kob.backend.pojo.UserOrder;
import com.kob.backend.service.user.order.GetOrderListAllService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RestController;

import java.util.List;

@RestController
public class GetOrderListAllController {
    @Autowired
    private GetOrderListAllService getOrderListAllService;

    @GetMapping("/order/getlist/all/")
    public List<UserOrder> getList() {
        return getOrderListAllService.getList();
    }
}